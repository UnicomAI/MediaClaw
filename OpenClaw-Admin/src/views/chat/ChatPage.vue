...
function parseRawContent(rawContent: ChatMessageContent[]): StructuredMessageView | null {
  ...
  for (const part of rawContent) {
    ...
    if (part.type === 'tool_result') {
      ...
      const extractedImages = extractImageFromText(contentText)
      images.push(...extractedImages)
...
const VIDEO_EXTS = /\.(mp4|webm|avi|mov|mkv|ogg|wmv|flv|m4v)$/i
const MEDIA_EXTS = /\.(png|jpg|jpeg|gif|webp|bmp|svg|mp4|webm|avi|mov|mkv|ogg|wmv|flv|m4v)$/i
...
onMounted(async () => {
  ...
  autoRefreshTimer = setInterval(() => {
    const key = chatStore.sessionKey.trim()
    if (key && !chatStore.loading && !chatStore.syncing) {
      chatStore.fetchHistory(key, { silent: true, clearError: false })
    }
  }, 1000)
})
                      ...
                      >
                        ...
                        <div v-if="entry.structured" class="structured-message-list">
                          ...
                          <div v-if="entry.structured.images.length" class="chat-images-container">
                            ...
                            >
                              <video
                                v-if="img.url && img.mediaType === 'video'"
                                :src="img.url"
                                class="chat-video"
                                controls
                                preload="metadata"
